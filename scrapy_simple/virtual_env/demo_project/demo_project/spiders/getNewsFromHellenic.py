...
class getDetail (scrapy.Spider):
    name = 'hellenic'
    start_urls = ['https://www.hellenicshippingnews.com/category/oil-energy/oil-companies-news']
    isContinue = False
    ...
    def parse(self, response):
        for i in range (1,16) :
            for n in response.xpath("//article[@class='item-list item_"+str(i)+"']"):
                ...
                date = n.xpath(".//p[@class='post-meta']/span/text()").extract_first()
                date_convert = datetime.datetime.strptime(date, "%d/%m/%Y").date()
                isContinue = False
