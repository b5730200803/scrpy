...
with open(os.path.abspath("C:\\Users\\supaK\\Desktop\\scrpy\\scrapy_simple\\virtual_env\\demo_project\\data.json")) as file:
    data = json.load(file)
    for line in data:
    ...
    lastDate_convert = datetime.datetime.strptime(
        lastDate, " %b %d, %Y").date()
...
class newsSpider (scrapy.Spider):
    ...
    def parse(self, response):
        for n in response.xpath("//div[@class='categoryArticle']"):
            date = n.xpath(
                ".//div[@class='categoryArticle__content']/p[@class='categoryArticle__meta']/text()").extract_first().split("|", 1)[0]
            date_convert = datetime.datetime.strptime(
                date, "%b %d, %Y at %H:%M ").date()
            ...
            # link เข้าไปในเนื้อหาข่าว
            url = n.xpath(
                ".//div[@class='categoryArticle__content']/a/@href").extract_first()
            isContinue = False
        ...
        next_page = response.xpath(
            "//div[@class='pagination']/a[@class='next']/@href").extract_first()
